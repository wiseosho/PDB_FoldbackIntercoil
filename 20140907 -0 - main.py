...
Risezero = 3.4
def Compare(a):
	now = a[len(a)-1] - a[len(a)-2]
	prev = a[len(a)-2] - a[len(a)-3]
	if now*prev < 0 :
		return 'turning'
...
# Sequence Initialization/ Duplex : Sequence of Characters.
	...
	for seq in sequence.readlines():
		...
		Duplex.append(list(seq.strip('\n'))); Duplen.append(len(list(seq)))
...
def Construct_FBI(maydai, duplex, Outer_K, Inner_K, Start, End, Twist):
	...
	if Twist == 0:
		...
		for j in range (8, 14):
			...
			for i in range (j, End):
				...
				origin = (maydai[j][44] + maydai[38-j][44])/2
				ang = 34.3*PII
...
def Export2PDB(Duplex, DuplexN, Duplen, mayday, Weave, Filename):
	...
	with open(st+Filename+".pdb", "w") as text:
		serN = 0 ; w = 1; n = 1 ; 
...
def main():
	...
	while (Helix[1] == 'Far'):
		Inner_K = 1 + Inner_K
		mayday[0] = Construct_FBI(mayday[0], Duplex[0], 0, 1, Start, End, 0 )
		parallel = np.dot(mayday[0][13][43]-mayday[0][13][44] , mayday[0][25][41] - mayday[0][25][44]) 
		distance = np.linalg.norm(mayday[0][13][44] - mayday[0][25][44])
		#print parallel
		if distance < 3:
			Helix[1] = 'Close'
	while (Helix[0] == 'Not Parallel'):
		Outer_K = Outer_K + 1
		mayday[0] = Construct_FBI(mayday[0], Duplex[0], 1, 0, Start, End, 0)
		parallel = np.dot(mayday[0][13][43]-mayday[0][13][44] , mayday[0][25][41] - mayday[0][25][44])
		if parallel <0:
			Helix[0] = 'Parallel'
	...
	mayday[0] = Construct_FBI(mayday[0], Duplex[0], 1, 0, Start, End, 1)	
	Export2PDB(Duplex, DuplexN, Duplen, mayday,Weave, 'FBI-loop')
	return 0
